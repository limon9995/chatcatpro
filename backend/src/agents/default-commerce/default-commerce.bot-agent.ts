...
import { WEBHOOK_SERVICE_TOKEN } from '../../message-queue/message.worker';
import type { WebhookServiceLike } from '../../message-queue/message.worker';
...
/**
 * Default commerce bot agent — handles the standard product/order pipeline
 * (OCR, draft orders, negotiation, product matching, AI intent, etc).
 * Extracted as-is from the former WebhookService._processMessageInner with
 * no logic changes. Owns its own image-buffer state; must not be imported
 * by any other BotAgent (see agents/bot-agent.interface.ts isolation note).
 */
@Injectable()
export class DefaultCommerceBotAgent implements BotAgent, OnModuleDestroy {
  readonly type = AgentType.DEFAULT;

  private readonly logger = new Logger(DefaultCommerceBotAgent.name);
  ...
  async handleMessage(page: any, psid: string, message: any): Promise<void> {
    ...
    if (page.businessBotOn) {
      if (!page.businessInfo) {
        await this.replyTracking.safeSend(
          token,
          psid,
      ...
      if (aiStatus === 'ok') {
        ...
        if (reply) {
          await this.replyTracking.safeSend(token, psid, reply);
      ...
      if (!activeOrder) {
        ...
        if (cancelledOrder) {
          ...
          if (this.isAdvanceRefundRequest(text)) {
            ...
            if (!hasAdvance) {
              await this.replyTracking.safeSend(token, psid, `❌ আপনার অর্ডার #${cancelledOrder.id} এ কোনো অগ্রিম পেমেন্ট পাওয়া যায়নি।`);
              return;
            }
            ...
            if (existingReturn) {
              ...
              await this.replyTracking.safeSend(token, psid, `✅ আপনার অগ্রিম ${sym}${totalAdvance} আগেই ফেরত দেওয়া হয়েছে। আর কোনো রিফান্ড সম্ভব নয়।`);
              return;
            }
            ...
            if (pendingReturn) {
              await this.replyTracking.safeSend(token, psid, `⏳ আপনার অগ্রিম ফেরতের অনুরোধ ইতিমধ্যে পাঠানো হয়েছে। শীঘ্রই যোগাযোগ করা হবে।`);
              return;
            }
            ...
            await this.replyTracking.safeSend(token, psid, `✅ আপনার অগ্রিম ফেরতের অনুরোধ পাঠানো হয়েছে। শীঘ্রই আপনার সাথে যোগাযোগ করা হবে।`);
            return;
          }
    ...
    if (structuredStep && page.orderModeOn) {
      ...
      if (result === null) {
        ...
        if (!stillExists) {
          ...
          await this.replyTracking.safeSend(token, psid, msg);
        }
        return;
      }
      if (typeof result === 'string') {
        await this.replyTracking.safeSend(token, psid, result);
        return;
      }
    }
    ...
    if (page.smartBotOn && aiAllowed && this.smartBot.isAvailable()) {
      ...
      if (reply !== false) {
        // Use replyTracking.safeSend so the reply is tracked → history gets saved
        await this.replyTracking.safeSend(token, psid, reply);
        return;
      }
    }
    ...
    if (!isStrongKeyword && aiAllowed) {
      ...
      if (businessContext) {
        ...
        businessContext.lastBotReply = this.replyTracking.peekByPsid(psid);
    ...
    if (intent === 'NEGOTIATION') {
      ...
      await this.replyTracking.safeSend(token, psid, reply + reminder);
      return;
    }
    ...
    if (draft && page.orderModeOn) {
      ...
      if (typeof result === 'string') {
        ...
        if (isRetry && aiEnabled) {
          ...
          if (fbResult.reply) {
            await this.replyTracking.safeSend(token, psid, fbResult.reply);
            return;
          }
        }
        await this.replyTracking.safeSend(token, psid, result);
    ...
    if (page.orderModeOn && this.isOrderStatusQuery(text)) {
      ...
      if (allOrders.length > 0) {
        await this.replyTracking.safeSend(token, psid, this.buildOrderStatusReply(allOrders));
        return;
      }
    }
    ...
    if (intent === 'DUAL_WEARING' || intent === 'DUAL_HOLDING') {
      ...
      if (!dualProduct) {
        await this.replyTracking.safeSend(token, psid, 'Product পাওয়া যায়নি।');
        return;
      }
      if (aiResult.reply) await this.replyTracking.safeSend(token, psid, aiResult.reply);
    ...
    if (intent === 'GREETING') {
      ...
      await this.replyTracking.safeSend(token, psid, greetReply);
      return;
    }
    ...
    if (intent === 'CATALOG_REQUEST') {
      if (aiResult.reply) {
        await this.replyTracking.safeSend(token, psid, aiResult.reply);
      }
      await this.sendCatalogFallback(token, psid, page);
      return;
    }
  ...
  private async handleCancel(
    ...
    await this.replyTracking.safeSend(page.pageToken, psid, reply);
  }
  ...
  private async handleRemoveItem(
    ...
    if (draft.items.length === 0) {
      await this.ctx.clearDraft(page.id, psid);
      await this.replyTracking.safeSend(page.pageToken, psid, '✅ Draft cancel হয়েছে।');
    } else {
      await this.ctx.saveDraft(page.id, psid, draft);
      await this.replyTracking.safeSend(
        page.pageToken,
  ...
  private async handleDraftEdit(
    ...
    if (
      ...
      await this.ctx.saveDraft(page.id, psid, draft);
      await this.replyTracking.safeSend(page.pageToken, psid, 'নতুন নাম দিন 💖');
      return true;
    }
    ...
    if (
      ...
      await this.ctx.saveDraft(page.id, psid, draft);
      await this.replyTracking.safeSend(page.pageToken, psid, 'নতুন ফোন নাম্বার দিন 💖');
      return true;
    }
    ...
    if (
      ...
      await this.ctx.saveDraft(page.id, psid, draft);
      await this.replyTracking.safeSend(page.pageToken, psid, 'নতুন ঠিকানা দিন 💖');
      return true;
    }
  ...
  private async handlePostOrderCancel(
    ...
    if (order.status === 'CANCELLED') {
      await this.replyTracking.safeSend(page.pageToken, psid, '❌ আপনার অর্ডারটি আগেই বাতিল হয়েছে।');
      return;
    }
    if (orderAgeHours > 24) {
      await this.replyTracking.safeSend(page.pageToken, psid, '⚠️ এই অর্ডারটি ২৪ ঘণ্টার বেশি পুরনো। বাতিল করতে সরাসরি আমাদের সাথে যোগাযোগ করুন।');
      return;
    }
  ...
  private async applyOrderFieldUpdate(
    ...
    await this.replyTracking.safeSend(pageToken, psid, `✅ আপনার ${fieldLabel} আপডেট হয়েছে: "${value}" 💖`);
    this.telegram
  ...
  private async handlePostOrderEdit(
    ...
    if (['SHIPPED', 'DELIVERED', 'CANCELLED'].includes(order.status)) {
      await this.replyTracking.safeSend(page.pageToken, psid, 'দুঃখিত 😔 এই পর্যায়ে অর্ডার পরিবর্তন করা সম্ভব নয়।');
      return;
    }
    ...
    if (!detected) {
      await this.replyTracking.safeSend(page.pageToken, psid, 'কোনটা বদলাতে চান? নাম, ফোন নম্বর, নাকি ঠিকানা?');
      return;
    }
    ...
    await this.replyTracking.safeSend(page.pageToken, psid, prompts[detected.field] || 'নতুন তথ্য লিখুন 💖');
  }
      ...
      const handleQueueFull = () => {
        void this.apiOcrFallback(entry.page, psid, entry.urls[0]).catch(() => {
          ...
          void this.botKnowledge
            ...
            .then((r) => this.replyTracking.safeSend(tok, psid, r))
  ...
  private async handleAudioMessage(
    ...
    await this.replyTracking.safeSend(token, psid, processingMsg);
    ...
    if (!transcribed) {
      ...
      await this.replyTracking.safeSend(token, psid, failMsg);
      return;
    }
    ...
    // constructing a synthetic message object and re-entering processMessage
    // (via WEBHOOK_SERVICE_TOKEN — same pattern MessageWorker uses to avoid
    // a circular import back to WebhookService)
    const syntheticMessage = { text: transcribed };
    await this.webhookService.processMessage(page, psid, syntheticMessage);
  }
